    ...
    function Graph(props) {
        ...
        _defineProperty(_assertThisInitialized(_this), "_onDragMove", function(ev, index, nodeList) {
            ...
            if (!_this.state.config.staticGraph) {
                ...
                draggedNode.y += _d3Selection.event.dy; // limit the node inside canvas boundaries

                var _this$state$config = _this.state.config,
                    width = _this$state$config.width,
                    height = _this$state$config.height;
                var adjustSize = draggedNode.size ? draggedNode.size / 2 : 0;
                var left = adjustSize;
                var right = width - adjustSize;
                var top = adjustSize;
                var bottom = height - adjustSize;
                draggedNode.x = _this._clamp(draggedNode.x, left, right);
                draggedNode.y = _this._clamp(draggedNode.y, top, bottom); // set nodes fixing coords fx and fy
        ...
        _defineProperty(_assertThisInitialized(_this), "_clamp", function(value, low, high) {
        ...
        _defineProperty(_assertThisInitialized(_this), "onKeyDownLink", function(event, source, target) {
            _this.props.onKeyDownLink && _this.props.onKeyDownLink(event, source, target);
        });
    ...
    _createClass(Graph, [
        ...
        },
        {
            ...
            value: function render() {
                var defs = (0, _graph4.renderGraphDefs)(this.state.config);
                var elements = (0, _graph4.renderWithBFS)(
                    this.state.nodes,
                    {
                    ...
                    this.state.d3Links,
                    {
                        ...
                        getLinkAriaLabel: this.props.getLinkAriaLabel,
                        // (source, target) => ariaLabel
