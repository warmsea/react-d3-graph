...
function updateNodeHighlightedValue(nodes, links, config, id) {
    ...
    var updatedNodes = _objectSpread(_objectSpread({}, nodes), {}, _defineProperty({}, id, node)); // when highlightDegree is 0 we want only to highlight selected node

    if (links[id] && config.highlightDegree !== 0) {
        updatedNodes = Object.keys(links[id]).reduce(function(acc, linkId) {
            var updatedNode = _objectSpread(
